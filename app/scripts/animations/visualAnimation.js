...
class VisualAnimation {
  ...
  applyEventListeners(elem, callback) {
    let fn = () => {
      if (!this.detectMob()) callback();
    };
    if (elem.addEventListener) {
      if ("onwheel" in document) {
        // IE9+, FF17+, Ch31+
        elem.addEventListener("wheel", this.onWheel(fn));
      } else if ("onmousewheel" in document) {
        // устаревший вариант события
        elem.addEventListener("mousewheel", this.onWheel(fn));
      } else {
        // Firefox < 17
        elem.addEventListener("MozMousePixelScroll", this.onWheel(fn));
      }
    } else {
      // IE8-
      elem.attachEvent("onmousewheel", this.onWheel(fn));
    }
  }
...
function init() {
  ...
  if (block.detectMob()) {
    ...
    block.applyEventListeners(
      document.body,
      block.changeClassOnShow.bind(block)
    );
